...
const DashboardPage = ({ setCurrentPage }) => {
  ...
  useEffect(() => {
    if (db && user && appId) {
      ...
      const unsubscribePlans = onSnapshot(
        ...
        (error) => {
          console.error(
            "Erro ao buscar planejamentos diários para o dashboard:",
            error
          );
  ...
  const analyzedData = useMemo(() => {
    ...
    const averageSentiment =
      sentimentTrend.length > 0
        ? (
            sentimentTrend.reduce((sum, item) => sum + item.sentiment, 0) /
  ...
  const handleGeneratePlanInsight = async (plan) => {
    ...
    try {
      ...
      let generatedText =
        "Não foi possível gerar o insight para este plano. Continue firme!";
      if (
        result.candidates &&
      ...
      showAlert(
        "Erro ao gerar insight do plano. Tente novamente mais tarde.",
        "error"
      );
    } finally {
      setInsightLoading(null);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[calc(100vh-150px)] bg-background-light dark:bg-gray-900 transition-colors duration-300">
        ...
        <div className="text-lg font-semibold text-gray-700 dark:text-gray-primary ml-4">
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6 sm:p-8 dark:bg-gray-800 dark:text-gray-primary transition-colors duration-300">
      ...
      {entries.length === 0 && dailyPlans.length === 0 ? (
        <p className="text-gray-500 text-center text-base sm:text-lg py-10 dark:text-gray-400">
          Nenhum dado encontrado. Comece a adicionar entradas no diário e
          planejamentos para ver suas perspectivas aqui!
        </p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          {/* SEÇÃO: Seu Bem-Estar Geral */}
          <div className="md:col-span-1 bg-gradient-to-br from-brand-primary to-gray-dark rounded-lg shadow-md p-6 text-white flex flex-col items-center justify-center relative overflow-hidden transform transition-all duration-300 hover:scale-105 hover:shadow-xl">
            <span className="absolute top-4 right-4 text-white text-opacity-30 text-5xl font-bold">
              ✨
            </span>
            <h3 className="text-xl sm:text-2xl font-bold mb-2">
              Seu Bem-Estar Geral
            </h3>
            <p className="text-4xl sm:text-5xl font-extrabold mb-3 leading-none">
              {averageSentimentScore}
            </p>
            <p className="text-sm opacity-90 text-center">
              (Média de sentimento do diário)
              {averageSentimentScore !== "N/A" &&
              parseFloat(averageSentimentScore) >= 1.0
                ? " - Ótimo trabalho, continue assim!"
                : averageSentimentScore !== "N/A" &&
                  parseFloat(averageSentimentScore) < 0
            ...
            {/* NOVO: Exibe totalEntries */}
          </div>

          {/* SEÇÃO: Foco e Progresso Recente (Último Planejamento) */}
          {lastPlan ? (
            <div className="md:col-span-1 bg-white rounded-lg border border-background-light shadow-md p-6 dark:bg-gray-700 dark:border-gray-600 transition-all duration-300 hover:shadow-xl">
              <h3 className="font-semibold text-lg sm:text-xl text-brand-primary mb-3 dark:text-accent-green">
                Foco do Último Plano
              </h3>
              <p className="text-sm text-gray-700 dark:text-gray-primary mb-2">
                Data:{" "}
                <span className="font-bold">
                  {new Date(lastPlan.date).toLocaleDateString("pt-BR")}
                </span>
              </p>
              <p className="text-sm text-gray-700 dark:text-gray-primary mb-3 line-clamp-2">
                Urgente e Importante:{" "}
                <span className="font-semibold">
                  {lastPlan.urgentImportant || "Nenhum item"}
                </span>
              </p>
              <p className="text-sm text-gray-700 dark:text-gray-primary mb-3 line-clamp-2">
                Tarefas principais:{" "}
                <span className="font-semibold">
                  {lastPlan.tasks || "Nenhuma tarefa."}
                </span>
              </p>
              <div className="flex gap-2 mt-4">
                <button
                  onClick={() => setCurrentPage("dailyPlanning")}
                  className="px-3 py-1 bg-accent-red text-white text-xs rounded-md hover:bg-accent-red-dark transition"
                >
                  Ver Todos os Planos
                </button>
                <button
            ...
            <div className="md:col-span-1 bg-white rounded-lg border border-background-light shadow-md p-6 dark:bg-gray-700 dark:border-gray-600 transition-all duration-300 hover:shadow-xl">
              ...
              <button
                onClick={() => setCurrentPage("dailyPlanning")}
                className="mt-4 px-3 py-1 bg-brand-primary text-white text-xs rounded-md hover:bg-brand-primary-dark transition"
              >
          ...
          <div className="bg-white p-4 sm:p-5 rounded-lg border border-background-light shadow-md transition-all duration-300 hover:shadow-xl dark:bg-gray-700 dark:border-gray-600 dark:text-gray-primary">
            <h3 className="font-semibold text-lg sm:text-xl text-brand-primary mb-3 dark:text-accent-green">
              ...
              <ResponsiveContainer width="100%" height={200}>
                ...
                >
                  ...
                  <XAxis
                    ...
                    textAnchor="end"
                    height={60}
                    style={{ fontSize: "0.75rem", fill: "#D1D5DB" }}
                  />
                  ...
                  <Bar dataKey="count" fill="#0E2F59" />{" "}
                  {/* Changed fill to brand-primary */}
                </BarChart>
              </ResponsiveContainer>
            ) : (
          ...
          <div className="md:col-span-2 bg-white p-4 sm:p-5 rounded-lg border border-background-light shadow-md transition-all duration-300 hover:shadow-xl dark:bg-gray-700 dark:border-gray-600 dark:text-gray-primary">
            <h3 className="font-semibold text-lg sm:text-xl text-brand-primary mb-3 dark:text-accent-green">
              Sua Jornada Emocional (Tendência de Sentimento)
            </h3>
            {sentimentTrend.length > 0 ? (
          ...
          <div className="md:col-span-2 bg-white p-4 sm:p-5 rounded-lg border border-background-light shadow-md transition-all duration-300 hover:shadow-xl dark:bg-gray-700 dark:border-gray-600 dark:text-gray-primary">
            ...
            <ul className="list-disc list-inside text-sm sm:text-base text-gray-700 space-y-1 dark:text-gray-300">
              <li>
                Mantenha a consistência no registro diário para melhores
                insights.
              </li>
              <li>
                Revise seu último planejamento diário e ajuste o foco, se
                necessário.
              </li>
              <li>
                Explore a página de Microcarreiras para alinhar suas habilidades
                aos seus valores.
              </li>
              <li>
                Seu bem-estar é prioridade. Faça um check-in sincero no seu
                diário hoje!
              </li>
            </ul>
          </div>
